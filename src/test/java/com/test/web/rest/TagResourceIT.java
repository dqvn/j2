...
import com.test.domain.Post;
import com.test.domain.Tag;
import com.test.repository.TagRepository;
import com.test.service.criteria.TagCriteria;
...
class TagResourceIT {
    ...
    @Test
    @Transactional
    void getAllTagsByEntryIsEqualToSomething() throws Exception {
        // Initialize the database
        tagRepository.saveAndFlush(tag);
        Post entry;
        if (TestUtil.findAll(em, Post.class).isEmpty()) {
            entry = PostResourceIT.createEntity(em);
            em.persist(entry);
            em.flush();
        } else {
            entry = TestUtil.findAll(em, Post.class).get(0);
        }
        em.persist(entry);
        em.flush();
        tag.addEntry(entry);
